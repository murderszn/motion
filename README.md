...
(images, videos, and GIFs) with WebGL shaders. Inspired by
[lumenshaders.vercel.app](https://lumenshaders.vercel.app/).
...
| Path | What it is |
|------|------------|
| `src/studio/` | TypeScript source for the studio — 19 typed modules |
...
└── studio/
    ...
    └── ui/
        ...
        ├── text.ts               # Text overlay, drag, canvas bake for export
...
**Keyboard shortcuts:** `space` pause/play · `r` randomize · `s` save PNG · `t` add text
...
One fullscreen triangle strip, one fragment shader containing all 14 presets
